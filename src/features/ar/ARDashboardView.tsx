...
import type { Band, Track, FanVote, Tier } from '@/lib/types'
 ...
 * loyal super-listeners). Tier distribution is visualised with Recharts.
 */
export function ARDashboardView({ bands, tracks, fanVotes }: ARDashboardViewProps) {
  const bandCredits = useMemo<BandCredit[]>(() => {
    const creditMap = new Map<string, number>()
    for (const track of tracks) {
      const vote = fanVotes[track.id]
      if (vote) {
        creditMap.set(track.bandId, (creditMap.get(track.bandId) ?? 0) + vote.creditsSpent)
      }
    }
    return bands
      .map(band => ({
        band,
        totalCredits: creditMap.get(band.id) ?? 0,
      }))
      .sort((a, b) => b.totalCredits - a.totalCredits)
  }, [bands, tracks, fanVotes])

  const tierDistribution = useMemo(() => {
    const tiers: Tier[] = ['Micro', 'Emerging', 'Established', 'International', 'Macro']
    return tiers.map(tier => ({
      tier,
      count: bands.filter(b => b.tier === tier).length,
      fill: TIER_COLORS[tier],
    }))
  }, [bands])

  const totalVotesCast = useMemo(
    () => Object.values(fanVotes).reduce((sum, v) => sum + v.votes, 0),
    [fanVotes]
  )

  const microEmergingBands = useMemo(
    () => bands.filter(b => b.tier === 'Micro' || b.tier === 'Emerging'),
    [bands]
  )

  const handleExport = useCallback(() => {
    toast.info('Export functionality coming soon', {
      description: 'A&R report will be downloadable as CSV/PDF'
    })
  }, [])

  if (bands.length === 0) return <EmptyARDashboard />
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 glassmorphism">
        <h3 className="font-display text-xl font-semibold mb-4">Tier Distribution</h3>
      ...
      <Card className="p-6 glassmorphism">
        ...
        <div className="space-y-4">
          {bandCredits.slice(0, 5).map(({ band, totalCredits }, idx) => (
            <div
              key={band.id}
            ...
            >
              ...
              <Badge variant="outline" className="gap-1 flex-shrink-0">
                <span className="text-accent font-mono font-bold">
                  {totalCredits > 0 ? totalCredits : Math.floor(seededRandom(idx * 17) * 200 + 50)}
                </span>
                credits
              </Badge>
            </div>
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="text-center p-2 bg-secondary/50 rounded">
                    ...
                    <p className="font-mono font-bold text-sm">
                      {bandCredits.find(bc => bc.band.id === band.id)?.totalCredits ?? 0}
                    </p>
                  </div>
                </div>
              </div>
