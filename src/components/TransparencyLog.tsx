...
import { Eye, Heart, Disc, UsersThree, Calculator, Check } from '@phosphor-icons/react'
...
export function TransparencyLog({ userId = 'current-user' }: TransparencyLogProps) {
  ...
  const userEntries = (transparencyLog || []).filter(entry => entry.userId === userId)
  const sortedEntries = [...userEntries].sort((a, b) => b.timestamp - a.timestamp)
  ...
  if (sortedEntries.length === 0) {
    return (
      <Card className="p-8 glassmorphism text-center">
        ...
        <h3 className="font-display text-lg font-semibold mb-2">No Voting History Yet</h3>
        <p className="text-sm text-muted-foreground">
          Your votes will appear here, showing exactly how they contribute to the final charts
        </p>
      </Card>
    )
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Complete mathematical breakdown of your {sortedEntries.length} votes
          </p>
        </div>
      </div>
